"""Real floor hardware driver, ported from the PoC `modules/`.
...
All hardware libraries are imported at the top of THIS module on purpose: it is
imported lazily by the node only when `driver:=real`, so a dev PC without the
buses/libs never touches it. The shared I2C bus is created once and handed to the
mux, wattmeter, and servo driver (one owner = serialized access, no mux race).
"""
import time
...
# Addresses / constants carried over from the PoC.
_MUX_ADDRESSES = [0x71 + i for i in range(8)]   # TCA9548A muxes, in wiring order
_SENSORS_PER_MUX = 4
_INA219_ADDRESSES = [0x40, 0x41, 0x44, 0x45]
_PCA9685_ADDRESS = 0x46
_SERVO_MIN_PULSE = 750
_SERVO_MAX_PULSE = 2250
_TLC_PIXELS = 12 // 3                            # TLC59711 drives 12 channels as 4 "pixels"
_PIR_POLL_S = 0.1


class _LedBoard(adafruit_tlc59711.TLC59711):
    """TLC59711 driven as 12 individual channels (PoC subclass)."""

    def set_channel(self, channel_index: int, value: float) -> None:
        max_channels = _TLC_PIXELS * 3
        v = int(max(0.0, min(1.0, value)) * 65535)
        adjusted = max_channels - 1 - channel_index          # PoC channel reversal
        start = self._buffer_index_lookuptable[adjusted]
        self._buffer[start] = (v >> 8) & 0xFF
        self._buffer[start + 1] = v & 0xFF


class RealFloorDriver(FloorDriver):
    def __init__(self, *, aht20=4, wattmeter=1, pir_pins=(18,), leds=4, servos=9,
        ...
        self._i2c = board.I2C()   # shared by AHT20 mux(es), INA219, PCA9685

        # --- AHT20 via TCA9548A mux(es) ---
        self._aht20 = []
        idx = 0
    ...
    def read_power(self):
        return [WattmeterReading(idx=i,
                                 shunt_voltage_mv=float(w.get_shunt_voltage_mV()),
